...
9. **Default model** → Configured instances can change it from `/setup` (`POST /setup/api/models/set`) or via `OPENCLAW_MODEL_ON_BOOT` (idempotent via `.wrapper-last-model-env` in `STATE_DIR`).
